...
import { Menu, Bell, LogOut, Users, UserCheck, Home, Package } from 'lucide-react'
    ...
    <div className="min-h-screen bg-dark-800 text-slate-200 flex">
      <aside className={`fixed lg:static inset-y-0 left-0 z-40 w-64 bg-dark-900 border-r border-dark-700 transform transition-transform duration-200 ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        ...
        <nav className="p-4 space-y-1">
          {navItems.map((item) => {
            ...
            return (
              <button key={item.id} onClick={() => { setActiveTab(item.id); setMobileMenuOpen(false) }} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition ${activeTab === item.id ? 'bg-brand-500 text-white' : 'text-slate-400 hover:bg-dark-700 hover:text-white'}`}>
                <Icon size={18} />
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-16 bg-dark-900/50 backdrop-blur border-b border-dark-700 flex items-center justify-between px-4 sticky top-0 z-30">
          <button className="lg:hidden p-2" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}><Menu size={20} /></button>
          <div className="flex items-center gap-4">
            <button className="p-2 hover:bg-dark-700 rounded-lg"><Bell size={18} /></button>
            <button onClick={() => signOut()} className="p-2 hover:bg-red-500/20 hover:text-red-400 rounded-lg transition"><LogOut size={18} /></button>
          </div>
        </header>
