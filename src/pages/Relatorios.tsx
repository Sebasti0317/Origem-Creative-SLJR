  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <h2 style={{ fontSize: 24, fontWeight: 'bold', margin: 0 }}>Relatorios</h2>
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button onClick={() => window.print()} style={{ padding: '8px 14px', background: '#6366f1', color: '#fff', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 13 }}>Imprimir PDF</button>
        </div>
      </div>
      ...
      <div style={{ background: '#1e293b', padding: 16, borderRadius: 10, border: '1px solid #334155', marginBottom: 20, display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
        ...
        {!isEducador && (
          <div style={{ flex: 1, minWidth: 150 }}>
            <label style={{ display: 'block', marginBottom: 4, color: '#94a3b8', fontSize: 12 }}>Modulo</label>
            <select 
              value={modulo} 
              onChange={(e) => setModulo(e.target.value)} 
              style={{ width: '100%', padding: 8, background: '#0f172a', border: '1px solid #334155', borderRadius: 6, color: '#e2e8f0', fontSize: 14 }}
            >
        ...
        {isEducador && (
          <div style={{ padding: '8px 12px', background: '#0f172a', border: '1px solid #334155', borderRadius: 6, color: '#6366f1', fontSize: 14 }}>
            Apenas Criancas (acesso restrito)
          </div>
        )}

        <div style={{ flex: 1, minWidth: 150 }}>
          <label style={{ display: 'block', marginBottom: 4, color: '#94a3b8', fontSize: 12 }}>Data Inicio</label>
          <input 
            type="date" 
            value={dataInicio} 
        ...
        <button 
          ...
          style={{ padding: '8px 16px', background: '#334155', color: '#e2e8f0', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 14 }}
        >
          Atualizar
        </button>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 20 }}>
        <div style={{ background: '#1e293b', padding: 16, borderRadius: 10, border: '1px solid #334155' }}>
          <div style={{ color: '#94a3b8', fontSize: 13 }}>Total de Registos</div>
          <div style={{ fontSize: 28, fontWeight: 'bold', color: '#6366f1' }}>{resumo.total}</div>
        </div>
        <div style={{ background: '#1e293b', padding: 16, borderRadius: 10, border: '1px solid #334155' }}>
          <div style={{ color: '#94a3b8', fontSize: 13 }}>Valor Total (Folha)</div>
          <div style={{ fontSize: 28, fontWeight: 'bold', color: '#10b981' }}>{resumo.valor.toLocaleString('pt-AO')} Kz</div>
        </div>
      </div>
      ...
      <div style={{ background: '#1e293b', borderRadius: 10, border: '1px solid #334155', overflow: 'hidden' }}>
        <div style={{ padding: 16, borderBottom: '1px solid #334155', display: 'flex', justifyContent: 'space-between' }}>
          <h3 style={{ margin: 0, fontSize: 16, color: '#e2e8f0' }}>Dados</h3>
          <span style={{ fontSize: 13, color: '#94a3b8' }}>{dados.length} resultados</span>
        </div>
        
        {dados.length === 0 ? (
          <div style={{ padding: 40, textAlign: 'center', color: '#64748b' }}>Sem dados para mostrar.</div>
        ) : (
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead style={{ background: '#0f172a' }}>
                <tr>
                  <th style={{ padding: 12, textAlign: 'left', borderBottom: '1px solid #334155', color: '#94a3b8' }}>Nome / ID</th>
                ...
                {dados.map((item: any, i: number) => (
                  <tr key={i} style={{ borderTop: '1px solid #334155' }}>
                    ...
                    <td style={{ padding: 10, color: '#64748b', fontSize: 12 }}>
                      ...
                      {item.salarioLiquido && `Liquido: ${parseFloat(item.salarioLiquido).toLocaleString()} Kz | `}
