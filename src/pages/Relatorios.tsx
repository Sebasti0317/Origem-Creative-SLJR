...
export default function Relatorios({ role }) {
  ...
  const gerarPDFCompleto = () => {
    ...
    const titulo = isFolha ? 'Folha Salarial' : isCriancas ? 'Relatório de Crianças' : 'Registo de Funcionários'
  ...
  // 🧾 IMPRIMIR FICHA INDIVIDUAL DA CRIANÇA
  const imprimirFichaCrianca = async (c) => {
    ...
    try {
      ...
      const nascimento = c.data_nascimento ? new Date(c.data_nascimento) : null
      const idade = nascimento ? Math.floor((new Date() - nascimento) / 3.15576e10) + ' anos' : 'N/D'
      const entrada = c.data_entrada || c.data_admissao || c.created_at

      const conteudo = `<!DOCTYPE html><html><head><title>Ficha - ${c.nome_completo}</title>
      <style>
      ...
      <div class="ficha">
        <div class="header"><h2>${nomeInst}</h2><p>Ficha Individual da Criança | ${new Date().toLocaleDateString('pt-PT')}</p></div>
        <div class="info-grid">
          <div class="info-item"><label>Nome Completo</label><span>${c.nome_completo || 'N/D'}</span></div>
          ...
          <div class="info-item"><label>Encarregado de Educação</label><span>${c.encarregado_nome || 'N/D'}</span></div>
          <div class="info-item"><label>Contacto do Encarregado</label><span>${c.encarregado_contacto || 'N/D'}</span></div>
          <div class="info-item"><label>Estado</label><span><span class="${c.ativo !== false ? 'badge-ativo' : 'badge-inativo'}">${c.ativo !== false ? 'ATIVO' : 'INATIVO'}</span></span></div>
        </div>
        ${c.observacoes ? `<div class="secao"><h3>Observações</h3><p style="font-size:13px;color:#475569;line-height:1.5;margin:0">${c.observacoes}</p></div>` : ''}
        <div class="footer">
          <div class="assinatura">Assinatura do Responsável</div>
          <div class="assinatura">Carimbo da Instituição</div>
        </div>
      ...
      toast.success('Ficha individual pronta para impressão!')
    } catch (e) {
      toast.dismiss(loading)
  ...
  // 🧾 IMPRIMIR RECIBO INDIVIDUAL DE FUNCIONÁRIO (Mantido)
