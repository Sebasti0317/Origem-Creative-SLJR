...
export default function Relatorios() {
  ...
  const carregarDados = () => {
    try {
      ...
      if (modulo === 'criancas' || isEducador) lista = JSON.parse(localStorage.getItem('criancas_db')||'[]')
      else if (modulo === 'funcionarios') lista = JSON.parse(localStorage.getItem('funcionarios_db')||'[]')
      else if (modulo === 'folha') lista = JSON.parse(localStorage.getItem('folha_salarial_db')||'[]')
      else {
      ...
      if (inicio || fim) lista = lista.filter(item => { const d = new Date(item.data_entrada||item.dataAdmissao||item.dataPagamento||Date.now()); return (!inicio||d>=inicio) && (!fim||d<=new Date(fim+'T23:59:59')) })
      setDados(lista)
      let totalVal = 0; lista.forEach(i => { if (modulo==='folha'||i.tipo==='Pagamento') totalVal += parseFloat(i.salarioLiquido||0) })
  ...
  return (
    <div style={{maxWidth:1200,margin:'0 auto'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
        ...
        <div style={{display:'flex',gap:8}}><button onClick={exportarCSV} style={{padding:'8px 14px',background:'#10b981',color:'#fff',border:'none',borderRadius:6,cursor:'pointer',fontSize:13}}>📥 CSV</button><button onClick={()=>window.print()} style={{padding:'8px 14px',background:'#6366f1',color:'#fff',border:'none',borderRadius:6,cursor:'pointer',fontSize:13}}>🖨️ PDF</button></div>
      </div>
      <div style={{background:'#1e293b',padding:16,borderRadius:10,border:'1px solid #334155',marginBottom:20,display:'flex',gap:12,flexWrap:'wrap',alignItems:'flex-end'}}>
        {!isEducador && (<div style={{flex:1,minWidth:150}}><label style={{display:'block',marginBottom:4,color:'#94a3b8',fontSize:12}}>Módulo</label><select value={modulo} onChange={e=>setModulo(e.target.value)} style={{width:'100%',padding:8,background:'#0f172a',border:'1px solid #334155',borderRadius:6,color:'#e2e8f0',fontSize:14}}><option value="geral"> Geral</option><option value="criancas"> Crianças</option><option value="funcionarios"> Funcionários</option><option value="folha"> Folha</option></select></div>)}
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))',gap:16,marginBottom:20}}>
        ...
        <div style={{background:'#1e293b',padding:16,borderRadius:10,border:'1px solid #334155'}}><div style={{color:'#94a3b8',fontSize:13}}>Valor Total</div><div style={{fontSize:28,fontWeight:'bold',color:'#10b981'}}>{resumo.valor.toLocaleString('pt-AO')} Kz</div></div>
      </div>
      <div style={{background:'#1e293b',borderRadius:10,border:'1px solid #334155',overflow:'hidden'}}>
        ...
        <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}>
          ...
          <tbody>{dados.map((item,i)=>(<tr key={i} style={{borderTop:'1px solid #334155'}}><td style={{padding:10,color:'#e2e8f0'}}>{item.nome||item.funcionarioNome||item.id?.slice(0,8)}</td><td style={{padding:10,color:'#94a3b8'}}>{item.tipo||modulo}</td><td style={{padding:10,color:'#94a3b8'}}>{new Date(item.data_entrada||item.dataAdmissao||item.dataPagamento||Date.now()).toLocaleDateString('pt-PT')}</td><td style={{padding:10,color:'#64748b',fontSize:12}}>{item.cargo&&`Cargo:${item.cargo}|`}{item.salarioLiquido&&`Líquido:${parseFloat(item.salarioLiquido).toLocaleString()}Kz|`}{item.genero&&`Género:${item.genero}`}</td></tr>))}</tbody>
        </table></div>}
      </div>
    </div>
  )
